...
def test_karate():
    ...
    res_autoval_p, res_autovect_p = u.test_power(K)
    assert(res_autoval_p)
    assert(res_autovect_p)

    res_autoval_d, res_autovect_d= u.test_deflation(K)
...
def test_3x3_nobase():
    ...
    #N = np.transpose(M)
    m_autoval_p, m_autovect_p = u.test_power(M)

    assert m_autoval_p
    assert m_autovect_p

    m_autoval_d, m_autovect_d = u.test_deflation(M)
